...
from optmentorwidgets import create_mermaid
create_mermaid(code='''
graph TD 
A[a] --> B[b] 
B --> C[c] 
...
class OPTWidget(DOMWidget, ValueWidget):
    ...
    srcprefix = Unicode('https://deep.cs.cityu.edu.hk/optmentor/opt-mentor/#mode=display&code=').tag(sync=True)
...
def create_optmentor(script="""print('Hello, World!')""", live=False, width=1100, height=700, offline=False):
    ...
    if live:
        ...
        srcprefix = "https://deep.cs.cityu.edu.hk/optmentor/opt-mentor/#mode=display&code="
    return OPTWidget(script=script, srcprefix=srcprefix, width=width, height=height)
